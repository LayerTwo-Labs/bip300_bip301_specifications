...
- The coinbase transaction contains more than one M3.
...
3. A block contains at most one `M3`, so no two `M6ID`s have the same inclusion
   block number. Let us call this chronologically sorted vector
   `M6IDS_chronological`.
...
If a coinbase transaction contains more than one `M3`, then the block MUST be
considered invalid.
