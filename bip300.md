...
USED_SIDECHAIN_SLOT_ACTIVATION_MAX_FAILS = USED_SIDECHAIN_SLOT_PROPOSAL_MAX_AGE - USED_SIDECHAIN_SLOT_ACTIVATION_THRESHOLD = 13_150
...
For every sidechain slot `S_i` in this list there is a vote
`V_i = M6ID | Abstain | Alarm`.
...
`ASN` MUST be computed from the BIP300 state as it stands after applying every
coinbase message at a lower output index than this M4.
...
The excess MUST NOT simply be ignored: an `A` longer than `ASN` can only come
from a miner whose active-slot list differs from the validator's, so the
elements that would survive truncation are misaligned too.

If the `A.len() < ASN.len()`, then the sidechain slots `ASN[A.len()]`, ...,
`ASN[ASN.len() - 1]` that `A` omits MUST be interpreted as `Abstain`.

An `A` of length `0` is valid, and means that every sidechain slot in `ASN`
abstains. Note that the `VOTES_TWO_BYTE` rule below cannot be satisfied by a
zero-length `A`, so this shape is available only to `VOTES_ONE_BYTE`.

A coinbase transaction that contains no M4 at all MUST be interpreted exactly as
one containing a `VOTES_ONE_BYTE` M4 with a zero-length `A`: every sidechain
slot in `ASN` abstains. Omitting the M4 is how a miner casts no votes in a block
whose coinbase activates a sidechain in a slot that was not already active,
since a shortened `A` is invalid there and a full-length `A` may not yet be
known when the coinbase is built.

If the `A.len() < ASN.len()` and an M2 at a lower output index than this M4
activated a sidechain in a slot that was not already active, then this M4 MUST
be considered invalid, and the whole block this M4 is included in MUST be
considered invalid as well, because that activation inserted a new slot number
into `ASN` at its sorted position, so an `A` sized against the pre-activation
`ASN` would set votes in the wrong slots.

An M2 that activates a sidechain in a slot that is _already_ active leaves `ASN`
unchanged — it replaces the sidechain occupying an existing entry rather than
adding one — so it shifts no index, and it MUST NOT invalidate a shorter `A`.
...
4. Get the actual `M6ID` value by indexing the sorted vector
   ...
   Ignoring it is not an option, for the same reason: the miner's
   `M6IDS_chronological` differs from the validator's, so the other votes in
   this slot are suspect too.
...
If a coinbase transaction contains duplicate `M3`s with exactly the same `S` and
`M6ID`, then the block MUST be considered invalid.
...
If an `M4` of version `VOTES_ONE_BYTE` or `VOTES_TWO_BYTE` omits trailing active
sidechain slots, and an M2 at a lower coinbase output index activated a
sidechain in a slot that was not already active, then the block MUST be
considered invalid.
