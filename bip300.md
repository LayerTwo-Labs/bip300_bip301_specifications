...
When we receive an `M1` we MUST store a sidechain proposal in the database:
...
If a sidechain proposal with exactly the same `S` and `D` already exists in the database, then this `M1` MUST be ignored.
Otherwise miners can reset the vote count on any sidechain proposal at will.

This is implemented in the `Bip300::handle_m1_propose_sidechain` method in the reference implementation.
...
We define `sidechain_proposal_age = height - proposal_height`, where `height`
is current block height, and `proposal_height` is the field of the
`SidechainProposal` we got from the database previously.
...
Then we set `new_sidechain_activated` to `true` if

```
sidechain_slot_is_used AND
vote_count > USED_SIDECHAIN_SLOT_ACTIVATION_THRESHOLD AND
sidechain_proposal_age < USED_SIDECHAIN_SLOT_PROPOSAL_MAX_AGE

OR

(NOT sidechain_slot_is_used) AND
vote_count > UNUSED_SIDECHAIN_SLOT_ACTIVATION_THRESHOLD AND
sidechain_proposal_age < UNUSED_SIDECHAIN_SLOT_PROPOSAL_MAX_AGE
```
